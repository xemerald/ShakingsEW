...
int skrep_list_init( void )
{
	if ( !SList ) {
		...
		if ( !SList ) {
			logit("e", "shakereport: Fatal! Station list memory initialized error!\n");
			return -1;
		}
	}
...
/*
 * skrep_list_search() - Insert the trace to the tree.
 * Arguments:
 *
 * Returns:
 ...
 */
STATION_PEAK *skrep_list_search( const char *sta, const char *net, const char *loc )
{
	STATION_PEAK *result = NULL;

/* Test if already in the tree */
	if ( (result = skrep_list_find( sta, net, loc )) == NULL ) {
	...
	/* Insert the station information into binary tree */
		if ( dl_node_append( (DL_NODE **)&SList->entry, result ) == NULL ) {
			logit("e", "shakereport: Error insert station into linked list!\n");
			goto except;
		}
		if ( tsearch(result, &SList->root, compare_snl) == NULL ) {
			logit("e", "shakereport: Error insert station into binary tree!\n");
			goto except;
		}
	/* */
		SList->count++;
...
/*
 * skrep_list_find() - Output the Trace that match the SNL.
 * Arguments:
 *
 * Returns:
 ...
 */
STATION_PEAK *skrep_list_find( const char *sta, const char *net, const char *loc )
{
	STATION_PEAK  key;
	STATION_PEAK *result;
 ...
 *    None.
 */
void skrep_list_walk( void (*action)( void *, const int, void * ), void *arg )
{
	int      i    = 0;
	DL_NODE *node = NULL;
 ...
 *
 */
CHAN_PEAK *skrep_list_chlist_search( STATION_PEAK *stapeak, const char *chan, const int pvalue_i )
{
	CHAN_PEAK *result  = NULL;

/* Test if already in the list */
	if ( (result = skrep_list_chlist_find( stapeak, chan )) == NULL ) {
	...
	/* */
		if ( dl_node_append( (DL_NODE **)&stapeak->chlist[pvalue_i], result ) == NULL ) {
			logit("e", "shakereport: Error insert channel into linked list!\n");
			goto except;
		}
		if ( tsearch(result, &stapeak->chlist_root, compare_chan) == NULL ) {
			logit("e", "shakereport: Error insert channel into binary tree!\n");
			goto except;
		}
	}
 ...
 *
 */
CHAN_PEAK *skrep_list_chlist_find( const STATION_PEAK *stapeak, const char *chan )
{
	CHAN_PEAK  key;
	CHAN_PEAK *result = NULL;
 ...
 *
 */
void skrep_list_chlist_delete( STATION_PEAK *stapeak, const char *chan, const int pvalue_i )
{
	CHAN_PEAK *target = skrep_list_chlist_find( stapeak, chan );
...
/*
 * skrep_list_end() - End process of stations' list.
 * Arguments:
 *   None.
 * Returns:
 *   None.
